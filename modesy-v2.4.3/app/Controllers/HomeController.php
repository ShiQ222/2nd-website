...
class HomeController extends BaseController
{
    ...
    public function index()
    {
        ...
        $data["brands"] = $this->commonModel->getBrands(30);
    ...
    public function products()
    {
        ...
        $data['pager'] = paginate($this->baseVars->perPageProducts, $numRows);
        $data['products'] = $this->productModel->getFilteredProductsPaginated($data['queryStringArray'], null, $data['customFilters'], $this->baseVars->perPageProducts, $data['pager']->offset);
    ...
    private function category($category)
    {
        ...
        $data['pager'] = paginate($this->baseVars->perPageProducts, $numRows);
        $data['products'] = $this->productModel->getFilteredProductsPaginated($data['queryStringArray'], $category, $data['customFilters'], $this->baseVars->perPageProducts, $data['pager']->offset);
    ...
    public function product($slug)
    {
        ...
        if (empty($data['product'])) {
            ...
            //custom fields
            $fieldModel = new FieldModel();
            $customFields = $fieldModel->getCustomFieldsByCategory($data['product']->category_id);
            $data['customFieldsDetails'] = array();
            $data['customFieldsAddditional'] = array();
            if (!empty($brand)) {
                if ($this->productSettings->brand_where_to_display == 1) {
                    array_push($data['customFieldsDetails'], ['name' => trans('brand'), 'value' => $brand]);
                } else {
                    array_push($data['customFieldsAddditional'], ['name' => trans('brand'), 'value' => $brand]);
                }
            }
            if (!empty($customFields)) {
                foreach ($customFields as $field) {
                    $fieldValue = getCustomFieldProductValues($field, $data['product']->id, selectedLangId());
                    if (!empty($fieldValue)) {
                        $item = [
                            'name' => @parseSerializedNameArray($field->name_array, selectedLangId()),
                            'value' => $fieldValue,
                        ];
                        if ($field->where_to_display == 1) {
                            array_push($data['customFieldsDetails'], $item);
                        } else {
                            array_push($data['customFieldsAddditional'], $item);
                        }
                    }
                }
            }
            ...
            $data['keywords'] = !empty($data['productDetails']->keywords) ? $data['productDetails']->keywords : '';
            //og tags
            $data['showOgTags'] = true;
    ...
    public function guestWishlist()
    {
        ...
        $data['pager'] = paginate($this->baseVars->perPageProducts, $numRows);
        $data['products'] = $this->productModel->getGuestWishlistProductsPaginated($this->baseVars->perPageProducts, $data['pager']->offset);
